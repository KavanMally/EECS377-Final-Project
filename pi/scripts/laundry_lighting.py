...
def on_message(mqttc, obj, message):
    #global bus_routine
    if "washers" in message.topic:
        washers = float(message.payload.split("Washers: ")[1].split(" of ")[0])
        print("washers available:", washers, "saturation", washers/11)
        msg = getMessage(1,washers/11.0, 1)
...
mqtt.subscribe("/sparti/laundry/washers", 0)
mqtt.subscribe("/sparti/laundry/dryers", 0)
